/* ==========================================================================
   figma-plugin-ds — self-hosted compiled script (IIFE build)
   Rebooted & maintained by Jean-Charles Amey.
   Drop this in with a <script> tag alongside figma-plugin-ds.css. Registers
   window.disclosure, window.selectMenu, and window.iconButton, each with
   .init() / .destroy(). Vanilla JS, zero dependencies.
   ========================================================================== */
...
(function () {
	...
	const iconButtonSelector = 'icon-button';

	//PRIVATE

	//toggles the pressed/selected state + keeps aria-pressed in sync. Only
	//attached to buttons that opt in via the data-toggle attribute — a plain
	//one-shot action icon-button (close, delete, etc.) should NOT get a
	//persisted "selected" look after being clicked, so it never gets this
	//listener; it only gets the keyboard handling below.
	const toggleHandler = function (event) {
		this.classList.toggle(iconButtonSelector + '--selected');
		this.setAttribute('aria-pressed', this.classList.contains(iconButtonSelector + '--selected'));
	};

	//Enter/Space activate the button the same way a native <button> would.
	//This fires a real click() rather than duplicating logic, so it triggers
	//both the toggle handler above (if attached) and any click handler the
	//consuming app registered itself — keyboard and mouse always stay in sync.
	...
	window.iconButton = {
		init: function () {
			let buttons = document.querySelectorAll('.' + iconButtonSelector);

			if (buttons) {
				buttons.forEach((button) => {
					//disabled buttons are inert: force tabindex="-1" (not just
					//dimmed via CSS opacity) and skip wiring up any listeners
					//at all, whether that's a real <button disabled> or a
					//<div class="icon-button icon-button--disabled">
					let isDisabled =
						button.disabled || button.classList.contains(iconButtonSelector + '--disabled');
					...
					//make it keyboard-reachable and announce it as a button,
					//unless the author already deliberately configured this
					//(e.g. tabindex="-1" to opt out, or their own role)
					...
					//opt-in toggle behavior — see toggleHandler comment above
					if (button.hasAttribute('data-toggle')) {
						if (!button.hasAttribute('aria-pressed')) {
							button.setAttribute(
								'aria-pressed',
								button.classList.contains(iconButtonSelector + '--selected')
							);
						}
						button.addEventListener('click', toggleHandler, false);
					}
				});
			}
		},
		...
		destroy: function () {
			...
			if (buttons) {
				buttons.forEach((button) => {
					...
					button.removeEventListener('click', toggleHandler, false);
				});
			}
		}
	};
